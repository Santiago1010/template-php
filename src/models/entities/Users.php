...
class Users implements iEntity {
	...
	public function __construct(?string $idUser = null, ?string $name = null) {
	...
	public function create(string $query): string {
    	$create = [
    		"createUser" => "INSERT INTO `{$this->table}` (id_user, name) VALUES (?, ?)"
    	];
    ...
    public function update(string $query): string {
    	$update = [
    		"updateUser" => "UPDATE `{$this->table}` SET name = ? WHERE id_user = ?"
    	];
