...
class Users implements iEntity {
    ...
    public function read(string $query): string {
    	$read = [
    		...
    		"readUser" => "SELECT * FROM $this->table WHERE id_user = :idUser",
    	];
    ...
    public function update(string $query): string {
    	$update = [
    		"updateUser" => "UPDATE {$this->table} SET name_user = :nameUser WHERE id_user = :idUser"
    	];
    ...
    public function delete(string $query): string {
    	$delete = [
    		"deleteUser" => "DELETE FROM {$this->table} WHERE id_user = :idUser"
    	];
